...
class SoundManager:
    ...
    def __init__(self, assets: AudioAssets) -> None:
        ...
        self.engine_chan = mixer.Channel(5)
    ...
    def handle_event(self, event, lander) -> None:
        if event.type == self.ENGINE_START_DONE:
            if lander.engine_on and self.assets.sounds.get("engine_loop"):
    ...
    # ---------- Engine control ------------
    def update_engine(self, lander, keys) -> None:
        if not lander.engine_on and not lander.crashed and keys[pygame.K_UP] and not lander.engine_out:
            ...
            if start:
                ...
                if loop:
                    ...
                    self.engine_chan.set_volume(Config.ENGINE_VOL)
        elif not lander.engine_on or not keys[pygame.K_UP] and not lander.crashed:
    ...
    def stop_all(self) -> None:
        """
        Stop all currently playing sounds and reset engine state.
        """
        # Immediately stop all sound channels
        pygame.mixer.stop()
        self.engine_chan.stop()    
        # Also stop any music playback
        try:
            pygame.mixer.music.stop()
        except AttributeError:
            pass

        # Stop engine-specific channel if set
        if hasattr(self, 'engine_chan') and self.engine_chan:
            self.engine_chan.stop()
            # Clear any end-event callback
            self.engine_chan.set_endevent()

        # Ensure every individual channel is stopped
