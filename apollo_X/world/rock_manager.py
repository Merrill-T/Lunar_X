...
class RockManager:
    ...
    def __init__(self, assets: "GfxAssets", terrain: "Terrain", lander: Any) -> None:
        self.assets = assets
        self.terrain = terrain
        self.lander = lander  # Required for base_coords and px_per_m
        self.topo: List[int] = []
    ...
    def _to_pixels(self, meters: float) -> float:
        return meters * self.lander.px_per_m
    ...
    def reset(self) -> None:
        ...
        # Convert to rock dict format for terrain.rocks
        self.terrain.rocks = [{
            "x": rx,
            "y": ry,
            "surface": img,
            "radius": max(img.get_width(), img.get_height()) // 2  # approx pixel radius
        } for rx, ry, img in self.rocks]
        #print(Terrain.rocks)
